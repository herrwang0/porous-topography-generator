...
def sunken_Antarctica_iceshelf(file_topoice, file_topobed, file_icethick, file_out,
                               ...
                               file_subgrid=None, inverse_depth=True, flood_depth=None, history=''):
    ...
    if file_subgrid is not None:
        ...
        maskc = np.full_like(csa, False)
        masku = np.full_like(usa, False)
        maskv = np.full_like(vsa, False)
        ...
        ush[masku], usa[masku], usl[masku] = dmask, dmask, dmask
        vsh[maskv], vsa[maskv], vsl[maskv] = dmask, dmask, dmask
